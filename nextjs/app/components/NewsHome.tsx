...
import type { getHomepage } from "../data/news";
      ...
      <section className="grid gap-8 border-b border-[var(--rule)] pb-8 lg:grid-cols-[1.35fr_0.65fr]">
        <article className="grid gap-5 lg:grid-cols-[0.95fr_1.05fr]">
          <a href={`/articles/${homepage.lead.slug}`} className="group block overflow-hidden bg-[#d9e1e2] [clip-path:polygon(0_0,100%_0,100%_88%,88%_100%,0_100%)]">
            <Image src={homepage.lead.image.src} alt={homepage.lead.image.alt} width={homepage.lead.image.width} height={homepage.lead.image.height} priority className="aspect-[5/3] h-full w-full object-cover transition duration-300 group-hover:scale-[1.02]" />
          </a>
            ...
            {homepage.topStories.map((story, index) => {
              ...
              return (
                <article className="border-b border-[var(--rule)] pb-4 md:border-b-0 md:border-r md:pr-4" key={story.title}>
                  ...
                  <Image src={image.src} alt={image.alt} width={image.width} height={image.height} className="mt-4 aspect-[5/3] w-full border border-[var(--rule)] object-cover" />
                </article>
              );
            })}
          </div>
