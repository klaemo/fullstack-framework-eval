...
export function CompactMenu({ sections }: { sections: string[] }) {
  ...
  return (
    <div className="md:hidden">
      ...
      {open ? (
        <nav id="compact-nav" className="absolute left-4 right-4 top-24 z-10 border border-[var(--rule)] bg-[var(--paper)] p-4 shadow-lg">
...
export function NewsletterForm() {
  ...
  return (
    ...
    >
      <label className="text-sm font-bold" htmlFor="rr-rsc-newsletter">
        Morning brief
      </label>
      <div className="flex gap-2">
        <input id="rr-rsc-newsletter" type="email" placeholder="email@example.com" className="min-w-0 flex-1 border border-[var(--rule)] bg-white px-3 py-2 text-sm" />
        <button className="bg-[var(--ink)] px-4 py-2 text-sm font-bold text-white" type="submit">
          Join
        </button>
      </div>
      <p className="text-xs font-bold text-[var(--muted)]">{joined ? "Briefing queued." : "One briefing, no noise."}</p>
    </form>
  );
}
